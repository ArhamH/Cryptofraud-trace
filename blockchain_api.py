...
blockchain_api.py
--------------------
Multi-chain transaction fetchers (EVM via Etherscan V2, Bitcoin via 
mempool.space, Solana via public RPC) with USD price normalization.
"""
...
def fetch_incoming_evm_txs(address, chain_id, api_key, native_symbol, limit=25):
    inbound = []
...
def fetch_inbound_summary(wallet, chain_key, api_key):
    chain = CHAINS[chain_key]
    family = chain["family"]
    if family == "evm":
        return fetch_incoming_evm_txs(wallet, chain["chain_id"], api_key, chain["native_symbol"])
    return []
